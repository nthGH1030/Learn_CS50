...
bool check(char* word)
{
    //go through the data structure of the trie created, follow the path
    //when reached the end, checked if is word = true

    //set the node pointer , cursor at the root of the diagram
    node *cursor = root;
    int count = 0;
    ...
    for(int i = 0; i < strlen(word); i++)
    {
        ...
        //check to see if the input string match the node
        if(cursor->children[index] != NULL)
        {
            //reset cursor to the node
            cursor = cursor->children[index];
            count++;
        }
    }
        //if the end is reached and is_word = true, return true
        if(count == strlen(word) && cursor->is_word == true)
        {
            return true;
        }
        else
